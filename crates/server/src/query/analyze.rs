...
impl QueryService {
    ...
    pub(super) fn run_analyze_pass(
        ...
        let represented_changes = if table.is_none() {
            components
        ...
        // leaves it alone.
        if table.is_none() {
            let _ = components.rows_changed_since_analyze.fetch_update(
                Ordering::AcqRel,
                Ordering::Acquire,
                |current| Some(current.saturating_sub(represented_changes)),
            );
        }
        Ok(())
    }
