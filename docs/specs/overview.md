...
Type mapping uses PostgreSQL OIDs `INTEGER` as `INT8` (`20`, size `8`), `TEXT` (`25`, size `-1`), `BOOLEAN` (`16`, size `1`), `DATE` (`1082`, size `4`), `TIMESTAMP` (`1114`, size `8`), `BYTEA` (`17`, size `-1`), and `UUID` (`2950`, size `16`). The simple query path always sends text: integers are decimal i64 strings, text is raw UTF-8, booleans are `t`/`f`, dates are `YYYY-MM-DD`, timestamps are `YYYY-MM-DD HH:MM:SS[.ffffff]`, bytea is hex `\x...`, uuid is the canonical `8-4-4-4-12` form, and null fields use length `-1`. (Binary `DATE` is an i32 day count from 2000-01-01; binary `TIMESTAMP` is an i64 microsecond count from 2000-01-01; binary `BYTEA` is the raw bytes; binary `UUID` is the 16 raw bytes.) The extended query protocol additionally supports binary parameters and results — `RowDescription` carries a per-field format code (`0` = text, `1` = binary) and `DataRow` carries the already-encoded wire bytes for that format.
