...
boundary, the retained `Checkpoint` marker carries the high-water mark so the
allocator boundary is still recovered even when data records below the checkpoint
have become logically excluded and their segments have been recycled.
...
covering a *committed* transaction whose `Commit` record is below the replay floor
...
unconditionally-recycled WAL could float it past an un-vacuumed aborted
transaction whose tuples survive (corruption). It re-derives the floor only in the
**no-snapshot fallback** (only a fresh replay-floor-zero WAL):
...
because no status record has yet been recycled.
