...
`QueryService::execute_sql`/`execute_prepared` run with no cancellation and default session identity; the connection uses `execute_simple_streamed` for simple queries and `execute_prepared_in_session_streamed`/`execute_prepared_cancelable_streamed` for extended `Execute` (in-transaction vs. autocommit, respectively), threading the connection's persistent `SessionSequenceState`, `SessionInfo` (startup user/database plus BackendKeyData process id), simple-protocol `SessionGucs`, and shared cancellation flag (an `Arc<AtomicBool>`) as `ExecutionContext.cancel`. The flag is cleared before each query and set when a `CancelRequest` for that backend arrives, so the in-flight query aborts with `SqlState::QueryCanceled` (SQLSTATE `57014`).
