...
- **`ALTER TABLE <t> SET (compression = 'none' | 'zstd')`** (`docs/specs/compression.md` §8): `run_alter_table_compression` runs the whole statement under the **exclusive** checkpoint guard (drains writers, like `VACUUM`/`CREATE INDEX` backfill), in this load-bearing order:
  ...
  7. **WAL flush (write-ahead of the page flush).** `wal.flush()` makes every rewrite FPI from step 6 durable before any of those pages are written back — required because `buffer_pool.flush_dirty_pages()` errors on a dirty page whose PageLSN exceeds the flushed WAL.
  8. **Flush + sync.** `buffer_pool.flush_dirty_pages()` then `store.sync_all()` re-encodes every dirtied page under the new config and makes it durable, then the guard releases. Command tag `ALTER TABLE`.
