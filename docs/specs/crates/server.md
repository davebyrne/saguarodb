...
11. Redo-all: replay every record with `LSN > checkpoint_lsn` (`WalManager::replay_from`). Physical page records replay under PageLSN gating regardless of transaction outcome; the CLOG hides aborted/in-flight versions. Before dispatch, `apply_redo` normalizes compressed FPIs to raw `FullPageImage` records using the dictionary resolver seeded in step 10. Logical catalog records for schemas, tables, indexes, sequences, views, dictionaries, ALTER metadata, schema rewrites, and relation generations replay only for committed transactions; records with a storage representation also replay through `RecoveryOperations`. A `DropTable` that encounters an internal FK dependency performs an on-demand streaming WAL pass for only that committed transaction's target set, bounded by the 16-bit catalog table-id domain, then discards the batch after the record; this lets cycles replay in WAL order without retaining drop history or changing the one-record-per-table format. Sequence value records replay unconditionally because sequence advancement is non-transactional. Skipped aborted/in-flight creates still reserve schema/table/view/index/sequence/dictionary IDs and carried table/index storage IDs; skipped `TruncateTable` and `UpdateTableSchema` reserve their replacement storage IDs. These reservations prevent orphan files or catalog IDs from being reused. Primary-key replay defers its derived identity-tree rebuild until replay and crashed-writer abort resolution finish. If replay applied records, repeat dictionary-reference validation so committed metadata cannot introduce an unresolved active dictionary.
...
- **`ALTER TABLE <child> ADD [CONSTRAINT name] FOREIGN KEY ...` / `DROP CONSTRAINT [IF EXISTS] name [RESTRICT]`**: FK ALTER allocates a maintenance xid and shared writer guard, then uses that xid's single deadlock-graph owner to converge schema/name/relation locks before taking the publication gate. ADD holds child `AccessExclusive` and parent `Share`; it re-resolves ordered columns and the declared PK/UNIQUE target, attaches one proposed constraint under the gate, scans every existing child row through the executor's shared outgoing validator, calls storage `update_table_schema`, and commits. DROP resolves the table-local name at execution, routes a matching PK name above, otherwise holds child `AccessExclusive` and parent `AccessShare`, removes only that FK without rewinding its ID allocator, and persists `UpdateTableSchema`. `IF EXISTS` suppresses only a missing named constraint; `CASCADE` is unsupported. Prepared ADD/DROP identities include both child and known parent and are revalidated after those relation locks converge under the publication gate. Any pre-flush validation, cancellation, storage, or WAL failure aborts and restores catalog/storage state; committed recovery applies the complete carried schema. Command tag `ALTER TABLE`.
...
`pg_get_constraintdef` renders primary-key, CHECK, and foreign-key definitions.
