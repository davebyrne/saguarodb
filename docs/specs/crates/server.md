...
11. Redo-all: replay every record with `LSN > checkpoint_lsn` (`WalManager::replay_from`) via `storage::apply_physical_redo` (PageLSN-gated; torn/missing pages are zeroed so a `FullPageImage`/`HeapInit` re-establishes them), regardless of the dirtying transaction's outcome — the CLOG (rebuilt at WAL open) decides visibility afterward, and an aborted/in-flight transaction's replayed versions are invisible (`mvcc.md` §8). Heap, identity-index, and catalog-index pages replay the same way. Before dispatch, `apply_redo` normalizes a `FullPageImageCompressed` record to a decompressed raw `FullPageImage` (`compression.decompress_fpi(codec, dict_id, payload, PAGE_SIZE)`, resolving `dict_id` against the resolver seeded in step 10) so `storage::apply_physical_redo` only ever handles the raw variant; an unresolvable `dict_id` at this point is a fatal structured recovery error (a normal crash never removes a referenced dictionary file — see `docs/specs/compression.md` §7's durability ordering). The `Commit`/`Abort`/`Checkpoint` markers are skipped (they are not page mutations). DDL records (`CreateTable`/`DropTable`/`CreateIndex`/`DropIndex`/`CreateSequence`/`DropSequence`) replay **only when their transaction is committed** (they mutate the durable catalog directly, not idempotent PageLSN-gated pages, so an aborted DDL's catalog change must not take effect). Table/index/sequence DDL records also replay through `RecoveryOperations`. `SequenceAdvance` and `SetSequenceValue` replay unconditionally into storage's sequence state because sequence values are non-transactional. `CreateDictionary`, `AlterTableCompression`, `AlterTableToast`, `TruncateTable`, `AlterTablePrimaryKey`, and `UpdateTableSchema` are classified alongside the other DDL records (`is_logical_catalog_record`) and replay under the same committed-only gate: a committed `CreateDictionary` (idempotently) re-saves the dictionary file and re-registers it; a committed `AlterTableCompression` applies `catalog.set_table_compression` then `storage.apply_set_table_compression`; a committed `AlterTableToast` applies `catalog.set_table_toast_metadata` then `storage.apply_set_table_toast_metadata`; a committed `TruncateTable` builds/applies the catalog storage-id update then calls `storage.apply_truncate_table` to publish the replacement generations; a committed `UpdateTableSchema` atomically applies the replayed table plus carried indexes to the catalog before applying the carried index schemas and table schema to storage; and a committed `AlterTablePrimaryKey` applies primary-key metadata while deferring the derived identity-tree rebuild until after replay and crashed-writer abort resolution. For skipped aborted/in-flight create records, recovery still reserves the table/index/sequence/dictionary IDs plus table/index storage IDs carried by `CreateTable` and `CreateIndex` schemas; skipped `TruncateTable` records reserve every carried replacement storage ID, and skipped `UpdateTableSchema` records reserve the carried table/index storage IDs. These reservations prevent replayed orphan page files, relation-generation files, or catalog IDs from being reused by a later object. If replay applied records, recovery repeats the dictionary-reference validation after step 11 so a committed metadata record replayed after the checkpoint cannot introduce an unresolved `active_dict_id` or `toast.active_dict_id`.
...
Every statement that reads or writes table data pairs its MVCC snapshot with a
storage relation-generation snapshot through `capture_consistent_snapshots`.
That helper takes the read side of `ServerComponents.relation_publish_gate`,
reads storage's relation epoch, captures and advertises the MVCC snapshot under
the active-transaction registry latch, then captures relation `Arc`s; if the
captured relation snapshot reports a different epoch, it drops the advertisement
and retries. This prevents lock-free readers from pairing an old MVCC snapshot
with post-truncate relation generations. Relation-swap DDL must take the write
side of that gate before the durable commit point and hold it until
catalog/storage generation publication finishes, then publish the storage
generation swap before deregistering its committed transaction. A reader can
therefore start before the DDL commits or after it publishes, but cannot capture
the old generation in the committed-but-not-published gap. After binding, query
orchestration compares every bound table's schema version with the captured
relation snapshot before producing rows or executing COPY/DML; a mismatch is
returned as retryable `SerializationFailure` so a statement cannot describe one
schema while scanning another relation generation.
...
- **Maintenance (`VACUUM [table]`, `TRUNCATE [TABLE] <name> [, ...]`, `ALTER TABLE <t> SET (compression = ...)`, `ALTER TABLE <t> SET (toast...)`, and `ALTER TABLE ... ADD/DROP PRIMARY KEY`)**: classified `StatementClass::Maintenance` (`statement_class`); they do not bind or plan, and are rejected inside an explicit transaction block — the rejection message is the generalized `"maintenance commands cannot run inside a transaction block"` (`FeatureNotSupported`), not a VACUUM-specific string; the block is poisoned to `'E'` exactly like a DDL-in-block rejection. `dispatch` (simple-query path) and the prepared-statement dispatch (`prepare_sql`/`execute_prepared_*`, which carries the raw parsed `Statement` unbound as `PreparedStatement.maintenance` since maintenance takes no parameters) both route through the single shared router `QueryService::run_maintenance(statement)`, which matches on the statement kind:
  ...
  - `Statement::Truncate { .. }` → `run_truncate`, which first acquires the **exclusive** checkpoint guard, then resolves and validates the complete ordered user-table target list before allocating/registering one maintenance `txn_id` or burning storage ids. The catalog builds every post-truncate schema without publishing; storage appends one existing logical `TruncateTable` WAL record per target and initializes every empty replacement heap/primary-index/secondary-index file. Before appending/flushing one `Commit`, the server takes `relation_publish_gate` write-side and holds it until catalog/storage publish the complete batch, so lock-free snapshot capture cannot observe a committed subset. Pre-commit failure rolls back every prepared storage generation. After durable batch publication, storage/buffer commit cleanup runs once, the xid is deregistered, lock waiters are awakened, checkpoint accounting runs once, and one best-effort retired-generation cleanup pass follows. Post-commit publication or cleanup failure is fatal because recovery will replay the committed batch. Command tag `TRUNCATE TABLE`.
...
- **`ALTER TABLE <t> SET (compression = 'none' | 'zstd')`** (`docs/specs/compression.md` §8): `run_alter_table_compression` runs the whole statement under the **exclusive** checkpoint guard (drains writers, like `VACUUM`/`CREATE INDEX` backfill), in this load-bearing order. Step 4's `wal.flush()` is the **durable commit point** (mirrors `autocommit_bound_write_with_guard`): steps 1-4 propagate an error normally as a statement error (nothing has committed yet — table resolution's `UndefinedTable` and a dict-training failure both land here). Steps 5-8 are **post-durable-commit cleanup**: any error there is routed to `fatal_after_durable_commit` (logs, best-effort WAL flush, `process::exit`) instead of being returned as a statement error, because the DDL already committed and misreporting it as failed would be worse than crashing. Step 9 (the checkpoint-account trigger) runs after the guard releases and is best-effort — `record_commit_and_maybe_checkpoint_after_durable_commit` logs any error rather than returning or crashing, exactly like every other commit path.
  ...
  4. **WAL + flush (durable commit point).** Append `CreateDictionary` (only if step 3 trained one), then `AlterTableCompression { table_id, compression, active_dict_id }`, then `Commit` — one combined `wal.flush()` after all three (immediate-commit DDL, like other DDL). The maintenance xid is registered while these records are prepared. Any error or statement cancellation before the commit flush appends `Abort`, rolls back storage/buffer bookkeeping, deregisters the xid, and removes the prepared dictionary from both the resolver and durable dictionary store; its allocated id remains burned.
...
- **`ALTER TABLE <t> ADD [CONSTRAINT name] PRIMARY KEY (cols...)` / `DROP PRIMARY KEY` / `DROP CONSTRAINT <pkey>`**: primary-key ALTER runs under the exclusive checkpoint guard as immediate-commit maintenance DDL. `ADD` requires the table to be a user table with no current primary key, resolves and de-duplicates the named columns, validates existing rows for NULLs, duplicate live keys, and live HOT-chain key divergence, reserves a catalog index id, appends `AlterTablePrimaryKey`, and builds the backing storage index before the commit without making the catalog PK/index visible to readers. After the commit is durable, SSI tuple reads and tuple writes retained for the table are conservatively promoted to relation-granularity tracking, storage rebuilds the table identity B-tree from heap rows with full-page-image redo and flushes that WAL, and only then the catalog atomically publishes the new primary-key metadata plus backing primary-key constraint index in one snapshot. `DROP` requires an existing primary key, optionally verifies the named constraint matches the primary-key constraint index, appends `AlterTablePrimaryKey { primary_key: [] }` and `DropIndex`, commits, atomically clears `TableSchema.primary_key` while removing the primary-key constraint index from the catalog, promotes retained table SSI tuple reads/writes, rebuilds hidden heap identity with flushed full-page-image redo, and marks the old storage index dropped. Dropped storage-index metadata remains scan-readable for already-planned statements but is excluded from future write maintenance. In both directions, failures before the commit flush abort the maintenance xid and restore the catalog snapshot; failures after the flush are fatal post-commit cleanup errors. The committed logical `AlterTablePrimaryKey` record is the recovery source of truth for crashes before a checkpoint containing the rebuild completes. Recovery applies catalog/storage metadata changes in WAL order, records the table id as needing a derived rebuild, then rebuilds the identity tree after the replay pass and after crashed writers have been marked aborted without appending recovery WAL. `DROP PRIMARY KEY` does not restore prior column nullability. Command tag `ALTER TABLE`.
- **COPY (`COPY <table> [(cols)] FROM STDIN | TO STDOUT`)**: classified `StatementClass::Copy(direction)`. `dispatch` captures the MVCC/relation snapshot before waiting for a writer guard, binds the statement (resolving table + columns and retaining the bound table schema), validates the bound target table schema version against the captured relation snapshot, and returns `ExecutionResult::BeginCopyIn`/`BeginCopyOut` — it does **not** execute — leaving the transaction slot unchanged. For **COPY FROM**, validation is strict for writes: the target relation must be present in the retained relation snapshot, the current catalog schema version must still match after the writer guard is held, and the relation epoch must still be current. Autocommit COPY FROM acquires and carries the shared writer guard before `CopyInResponse`; COPY FROM inside a transaction lazily installs that guard on the open transaction before the response. The bound `CopyJob` carries the resolved `TableSchema` into the streaming worker, so after `CopyInResponse`/`CopyOutResponse` is sent the worker does not re-read the live catalog for COPY column names, input types, or defaults. The connection loop then drives the COPY sub-protocol (see `docs/specs/copy.md` §5); COPY is rejected in the extended query protocol (`prepare_sql`). For **COPY FROM**, the loop sends `CopyInResponse`, spawns a blocking task (`run_copy_in_stream`) that owns the transaction — a fresh autocommit transaction that registers an xid and uses the carried snapshot/guard, or, inside a `BEGIN` block, the open transaction with no commit (mirrors `run_bound_in_transaction`) — and forwards `CopyData` into a bounded channel; `CopyDone` commits, while `CopyFail`/a row error/disconnect abort (status-based, like any other failure). For **COPY TO**, a blocking producer (`run_copy_out_stream`) scans under a read snapshot (autocommit) or the transaction's snapshot and streams `CopyData` frames out, then `CopyDone`. Command tag `COPY n`; a mid-stream error sends `ErrorResponse` (no `CopyDone`). `ReadyForQuery` is emitted only after the inbound stream is drained to its terminator, so the protocol stays in sync.
...
- **Writers run concurrently.** A write transaction acquires the **SHARED** writer guard (`begin_writer`) **lazily** on its first write statement and holds the owned guard on the `Session` for the whole write-transaction, releasing it at COMMIT/ROLLBACK/disconnect. Many writers hold it at once; write-write safety comes from per-row conflict coordination and the per-index / per-heap structural latches in `storage`, not from this lock. In-progress row-lock conflicts block through the server `LockManager`; timeout-based deadlock detection returns `SqlState::DeadlockDetected` (`40P01`), cancellation returns `SqlState::QueryCanceled` (`57014`), and committed-superseded conflicts return `SqlState::SerializationFailure` (`40001`). Autocommit DML = acquire the shared guard for the one statement, release at the implicit commit.
...
DDL follows the same allocate/execute/commit-or-abort sequence as an autocommit write, but it acquires the exclusive checkpoint guard instead of the shared writer guard. Catalog and storage mutations are part of the same statement-level commit. `CREATE TABLE IF NOT EXISTS` still validates the requested table definition shape, then returns the normal command tag without catalog/storage/WAL mutation when the table already exists. `CREATE TABLE ... WITH (fillfactor = 10..100)` accepts and validates the PostgreSQL compatibility option but intentionally does not persist it or change page occupancy. Multi-target `DROP TABLE [IF EXISTS]` resolves and validates the complete ordered target list before mutation, skips absent targets only with `IF EXISTS`, and applies every drop under one statement transaction; each object retains its existing logical WAL record. `CREATE TABLE` with `SERIAL` creates the owned sequences before the table and stores ordinary `ColumnDefault::Nextval` defaults. `CREATE TABLE` with a `TEXT` or `BYTEA` column creates a hidden TOAST relation in the catalog and installs both the base and hidden schemas in storage under the same statement context; the hidden relation is not resolvable by user table name. `DROP TABLE` emits sibling `DropSequence` records for owned sequences and cascades catalog/storage metadata to the linked hidden TOAST relation. `CREATE INDEX` may publish the catalog index before storage publishes the new relation generation, but `SchemaOperations::create_index` publishes storage metadata only after the secondary tree is initialized and backfilled. Schema-evolution ALTER TABLE emits `UpdateTableSchema`; ADD/DROP column rewrites switch to fresh storage ids and reinsert transformed visible rows, while renames update metadata only. `CreateTable`, `DropTable`, `CreateSequence`, and `DropSequence` WAL replay update both catalog and storage; `CreateIndex` and `DropIndex` update catalog, storage metadata, and index pages through their normal recovery paths; committed `UpdateTableSchema` atomically updates catalog table/index metadata before publishing storage table metadata and any carried catalog-index schemas; `AlterTablePrimaryKey` updates table metadata and triggers a deferred recovery rebuild of the derived storage identity tree. Normal DDL execution must restore the previous catalog state if storage mutation, WAL append, or WAL flush fails before the commit record is durable; DML rollback does not restore a catalog snapshot.
...
- `TRUNCATE a, missing` and a list with a late wrong-object target fail under the
  exclusive guard before allocating an xid or storage ids and leave every table
...
- `VACUUM ANALYZE` and `VACUUM ANALYZE <table>` run the ordinary VACUUM pass,
  return the `VACUUM` command tag, and create no persistent statistics.
