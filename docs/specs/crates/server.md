...
- **Maintenance (`VACUUM [table]`, `TRUNCATE [TABLE] <table>`, `ALTER TABLE <t> SET (compression = ...)`, `ALTER TABLE <t> SET (toast...)`, and `ALTER TABLE ... ADD/DROP PRIMARY KEY`)**: classified `StatementClass::Maintenance` (`statement_class`); they do not bind or plan, and are rejected inside an explicit transaction block — the rejection message is the generalized `"maintenance commands cannot run inside a transaction block"` (`FeatureNotSupported`), not a VACUUM-specific string; the block is poisoned to `'E'` exactly like a DDL-in-block rejection. `dispatch` (simple-query path) and the prepared-statement dispatch (`prepare_sql`/`execute_prepared_*`, which carries the raw parsed `Statement` unbound as `PreparedStatement.maintenance` since maintenance takes no parameters) both route through the single shared router `QueryService::run_maintenance(statement)`, which matches on the statement kind:
  ...
  - `Statement::Truncate { .. }` → `run_truncate`, which resolves exactly one user table, acquires the **exclusive** checkpoint guard, allocates/registers a maintenance `txn_id`, asks the catalog to prepare fresh storage ids and build the post-truncate schemas without publishing them, and calls `storage.prepare_truncate_table` to append the logical `TruncateTable` WAL record and initialize empty replacement heap/primary-index/secondary-index files. Before appending/flushing the `Commit`, it takes `relation_publish_gate` write-side and holds it until catalog/storage publication finishes, so lock-free snapshot capture cannot observe the committed-but-not-published gap. After the commit flush, it calls `catalog.apply_truncate_table`, `storage.publish_truncate_table`, storage/buffer commit cleanup, deregisters the xid, wakes waiters, releases the guards, triggers checkpoint accounting, and runs one best-effort retired-generation cleanup pass. Broad orphan-file cleanup remains a checkpoint/startup responsibility because those contexts exclude concurrent relation-file publishers. Errors before the commit flush are statement errors after pre-durable rollback; errors after the commit flush are routed to `fatal_after_durable_commit` because recovery will replay the committed truncate. Command tag `TRUNCATE TABLE`.
...
writer/exclusive concurrency guards and snapshot-exclusion waits use short timed
