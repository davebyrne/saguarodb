...
| `WindowOp` | Stable-sorts by synthesized `PARTITION BY ... ASC NULLS LAST` keys followed by the declared window ordering, buffers one partition at a time in a spill tape sharing the sorter's operator budget, and emits source columns plus ranking/distribution, `ntile`, offset/value functions, and all aggregates in expression order. Forward-only peer/frame probes implement checked, clamped `ROWS` bounds and peer- or threshold-based `RANGE` bounds, including direction-aware numeric and temporal offsets. Window aggregates reuse `AggregateState`: whole-partition results are cached, unbounded-preceding frames grow through snapshots, moving-head frames recompute from a forked frame cursor, and collect aggregates always recompute. Value lookups and recomputation fork independently positioned tape readers, so disk spill preserves semantics. Output preserves identity and sorted stable input order |
...
- Window operator resets partitions, recognizes peers (including NULL peers), computes ranking/distribution and uneven `ntile`, applies lag/lead direction/default/NULL rules, preserves stable emission order and identity, and produces identical rows under forced spill. Unit coverage includes cancellation and child open/next/close failures; `e2e_window_*` covers grouped inputs, chained specifications, window-result ordering, derived tables/subqueries, `EXPLAIN ANALYZE`, `22014`, spill, ROWS/RANGE frames, and aggregates over frames.
