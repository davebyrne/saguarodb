...
- Validate each `CREATE TABLE` foreign key against the proposed source columns
  ...
  execution assigns the table ID. Omitted target columns select the PK. Explicit
  targets must exactly match an ordered declared PK/UNIQUE constraint, and paired
  declared types/`PgType`/length metadata must be identical. The bound FK list is
  threaded unchanged through logical and physical CREATE plans. Unqualified
  parents use the ordinary search path with the proposed table acting as a
  self-reference candidate only at its own schema position. Existing parents are
  included in prepared schema identities. See `docs/specs/foreign-keys.md`.
...
A top-level `SELECT` binds to `BoundStatement::Query(BoundQuery)`. Binding a `BoundQuery` binds its body (a `BoundSelect`) and, for a `SELECT` body, binds the query-level `ORDER BY` against that block's output columns (the `ORDER BY`/`DISTINCT` validation is coupled and stays together). Logical planning lowers the body, then applies the wrapper's `ORDER BY`/`LIMIT`/`OFFSET`; the aggregate-context `ORDER BY` rewrite stays with the body because it depends on the body's `group_by`/aggregates. A row-locking clause binds to `BoundRowLock { table, mode, wait_policy }` and is accepted only on a top-level SELECT over exactly one base table, optionally naming that table or its alias with `OF`. DISTINCT, grouping/aggregation, joins, views, derived/system relations, set operations, VALUES, and subqueries in the locked query are rejected. Planning places `LockRows` after the snapshot filter and optional sort but before LIMIT/OFFSET. It retains scan identity, carries the original predicate for latest-version recheck, and evaluates the projection only after the latest row is locked. `BoundSelect` (without the query-level modifiers) is also used directly as the source for `UPDATE` and `DELETE`, preserving filters and row identity through execution.
...
- `logical_plan` and `physical_plan` do not accept `BoundStatement::Explain` directly; callers must extract and plan the inner bound statement.
