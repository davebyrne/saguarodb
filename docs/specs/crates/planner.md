...
`PlanNodeLayout` assigns execution-local, zero-based node IDs in deterministic pre-order: root first, a unary child next, binary left before right, and Apply input before subplan. The IDs are stable for an unchanged physical tree but are not durable or catalog identifiers. The immutable layout exposes only its node `id()` and checked `child(index)` lookup; IDs are explanation/execution metadata and are not stored in `PhysicalPlan` variants.
