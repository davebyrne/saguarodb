...
- `EXPLAIN SELECT ...`, `EXPLAIN ANALYZE SELECT ...`, and `EXPLAIN (ANALYZE [TRUE|FALSE]) SELECT ...`. The AST carries the option as `Statement::Explain { analyze, statement }`, and only a `SELECT` inner statement is accepted. Parenthesized options accept exactly one case-insensitive `ANALYZE`; a bare option means true, while boolean literals and PostgreSQL boolean text spellings use `common::parse_bool_text`. Duplicate or unknown options, invalid values, `VERBOSE`, `FORMAT`, `QUERY PLAN`, `ESTIMATE`, and non-query inner statements are rejected as unsupported.
