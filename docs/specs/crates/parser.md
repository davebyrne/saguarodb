...
pub enum Statement {
    ...
    // ALTER form; owns the whole ALTER namespace (every other `ALTER ...` input is
    // rejected here, not left to sqlparser). `table` is lowercase-normalized.
...
- `ALTER TABLE <name> SET (compression = 'none' | 'zstd')` (`docs/specs/compression.md` §4, §8) is the only supported `ALTER` form, and `try_parse_alter_table` **owns the entire `ALTER` namespace**: like `VACUUM`, it intercepts the statement before sqlparser (tokenizing with sqlparser's tokenizer) rather than falling through to sqlparser's own `ALTER TABLE` grammar, because that grammar covers far more than this one supported form. A leading `ALTER` word that is not followed by exactly `TABLE <name> SET (compression = <value>)` — a different `ALTER TABLE` sub-form, `ALTER INDEX`, `ALTER SEQUENCE`, anything else — is rejected with `SqlState::FeatureNotSupported`, never left to sqlparser, so an otherwise well-formed `ALTER` a future sqlparser version might parse is still uniformly rejected. A genuinely malformed option list (missing `(...)`, missing `=`, an unparsable value token) is instead `SqlState::SyntaxError`. The table name is lowercase-normalized (the unquoted-identifier rule); the compression value follows the same string-or-identifier, case-insensitive parsing as the `CREATE TABLE ... WITH (compression = ...)` clause, and an unrecognized codec is `SqlState::FeatureNotSupported`. Like `VACUUM`, `ALTER TABLE` does not bind/plan — it is a maintenance command the server dispatches separately (`docs/specs/crates/server.md`).
...
- Convention for a recognized-but-unsupported *option value* (the `CREATE TABLE
  ... WITH (compression = ...)` clause, and `ALTER TABLE ... SET (compression =
  ...)`): a malformed clause shape (missing parens, missing `=`, an
  unparsable value token) is `SqlState::SyntaxError` — the input is not
  well-formed SQL for the recognized clause. A well-formed clause naming an
  option or value SaguaroDB does not implement (an unknown option key, or a
  codec other than `none`/`zstd`) is `SqlState::FeatureNotSupported` instead —
  the input parses but names something out of scope. The same split governs
  the `ALTER` namespace: a malformed `SET (compression = ...)` option list is
  `SyntaxError`; any other well-formed `ALTER ...` form is `FeatureNotSupported`.
...
- Parses `CREATE TABLE ... WITH (compression = 'zstd' | 'none' | zstd)` (quoted and bare-identifier forms) into `Statement::CreateTable.compression`; an absent `WITH` clause parses to `None`; an unsupported codec value is `FeatureNotSupported`; a duplicate `compression` option is `SyntaxError`.
- Parses `ALTER TABLE <name> SET (compression = ...)` (including mixed-case keywords and an optional trailing `;`) into `Statement::AlterTableSetCompression`; `ALTER INDEX ...` and every other `ALTER` form is `FeatureNotSupported`; an unsupported codec value is `FeatureNotSupported`; a malformed option list (e.g. missing `(...)`) is `SyntaxError`.
