...
The dedicated `TRIM(expr)`, `SUBSTRING(expr [FROM start] [FOR length])` (and the comma form `SUBSTRING(expr, start[, length])`), and `CEIL(expr)` / `FLOOR(expr)` grammar is normalized into ordinary `Function { name: "trim" | "substring" | "ceil" | "floor", ... }` calls so the binder treats them uniformly. `SUBSTRING` requires a start argument; `TRIM` with `LEADING`/`TRAILING`/`BOTH` or trim characters is unsupported; the `CEIL(expr TO <field>)`/scale forms are unsupported. (`CEILING` is not a sqlparser keyword, so it arrives as a plain `ceiling` function call, which the binder treats like `ceil`.)
