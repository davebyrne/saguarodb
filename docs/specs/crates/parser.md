...
- `ALTER TABLE <name> SET (...)` owns the entire `ALTER` namespace: like `VACUUM`, `try_parse_alter_table` intercepts it before sqlparser (tokenizing with sqlparser's tokenizer) rather than falling through to sqlparser's own broad `ALTER TABLE` grammar. The executed form remains `ALTER TABLE <name> SET (compression = 'none' | 'zstd')` (`Statement::AlterTableSetCompression`, `docs/specs/compression.md` §4, §8). The parser also accepts the TOAST option keys documented for `CREATE TABLE ... WITH (...)` and returns `Statement::AlterTableSetOptions { options: TableOptionPatch }`; the server classifies that statement as maintenance but rejects execution with `FeatureNotSupported` until TOAST ALTER execution is implemented. A leading `ALTER` word that is not `TABLE <name> SET (...)` — `ALTER INDEX`, `ALTER SEQUENCE`, `ALTER TABLE ... ADD COLUMN`, renames, and so on — is rejected with `SqlState::FeatureNotSupported`. Malformed option lists, duplicate recognized keys, and unrecognized option keys are `SqlState::SyntaxError`; unsupported enum values are `SqlState::FeatureNotSupported`; out-of-range TOAST numeric values are `SqlState::InvalidParameterValue`. The table name is lowercase-normalized (the unquoted-identifier rule). Like `VACUUM`, `ALTER TABLE` does not bind/plan — it is a maintenance command the server dispatches separately (`docs/specs/crates/server.md`).
...
- Parses `ALTER TABLE <name> SET (...)` with any TOAST option into `Statement::AlterTableSetOptions`; execution is intentionally rejected by the server until TOAST ALTER is implemented.
